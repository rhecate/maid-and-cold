...
@onready var roomsize = roomspace.shape.get_rect()
...
func spawn_toy():
	...
	var rndX = rng.randf_range(roomsize.position.x, roomsize.end.x)
	var rndY = rng.randf_range(roomsize.position.y, roomsize.end.y)
	toy.global_position = Vector2(rndX, rndY)
	print("--", rndX, " , ", rndY, "--")
	await get_tree().create_timer(0.5).timeout
	toy.queue_free()
	spawn_toy()
